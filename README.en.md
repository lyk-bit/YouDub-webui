...
YouDub WebUI turns a YouTube, Bilibili, or local video into a target-language version. It downloads or imports the source, transcribes and translates it, then adds subtitles, generates translated voiceover, or does both according to the task's output selection. The final video can be played or downloaded from the web UI.

The most mature path is **YouTube English -> Chinese dubbing**. YouTube links, Bilibili links, and local videos can all choose among the `English -> Chinese`, `Japanese -> Chinese`, and `Chinese -> English` directions per task (defaults: English -> Chinese for YouTube, Chinese -> English for Bilibili).
...
| `VAD_ENABLED` / `VAD_THRESHOLD` / `VAD_MIN_SPEECH_MS` / `VAD_MIN_SILENCE_MS` | silero-vad voice-activity detection in the sentence-fix stage: utterance timestamps are clamped to detected speech regions so silence and non-verbal vocals (moans, breathing, and similar) re-emerge as inter-sentence gaps, which the audio-merge stage then backfills with the original vocal track (Kotoba-Whisper's contiguous timestamps otherwise swallow those regions into neighboring sentences). `VAD_ENABLED` defaults to on (set `off` to disable); `VAD_THRESHOLD` is the speech-probability threshold, default `0.5` — raise it to route more non-verbal vocals into the backfill gaps (too high may drop soft speech); `VAD_MIN_SPEECH_MS` / `VAD_MIN_SILENCE_MS` are the minimum speech/silence durations, defaults `250` / `100`. Skipped automatically when silero-vad is missing or fails to load. |
| `SPEAKER_DIARIZATION` | Run speaker diarization after ASR, default `true`. Detected speakers are written to each sentence's `speaker` field: for multi-speaker videos translation requests automatically carry a "说话人N：" tag (helping the LLM resolve perspective, address terms, and register), and VoxCPM synthesis keeps a distinct voice per speaker. When disabled, every sentence is attributed to speaker `1`. |
| `DIARIZATION_MODEL` | Hugging Face model used for speaker diarization, default `pyannote/speaker-diarization-3.1`. |
| `PYANNOTE_HF_TOKEN` | Hugging Face access token for the pyannote model. The default model is a gated repo: accept its terms on the [model page](https://huggingface.co/pyannote/speaker-diarization-3.1) before pasting the token. Without a token (and without an `HF_ENDPOINT` mirror) diarization is skipped automatically and tasks are unaffected. |
...
8. Return to the home page and submit a YouTube URL, Bilibili URL, or local video.
   - Under `Output content`, choose `Subtitles only`, `Translated voice only`, or `Subtitles and translated voice`. Subtitle-only output preserves the original audio and skips dubbing; voice-only output does not burn subtitles.
   ...
   - All three sources can pick `English -> Chinese`, `Japanese -> Chinese`, or `Chinese -> English` under `Translation direction`; without a manual choice the per-source default applies (English -> Chinese for YouTube, Chinese -> English for Bilibili). For local videos the direction also determines the optional subtitle's target language; for example, `Japanese -> Chinese` treats the uploaded SRT as Chinese subtitles.
...
YouTube / Bilibili URL
  ...
  -> Branch by output content:
     - Subtitles only: preserve original audio and burn translated subtitles
     - Voice only: generate VoxCPM2 voiceover and mix it with background audio, without subtitles
     - Both: generate and mix voiceover, then burn translated subtitles
...
- **Multiple translation paths**: YouTube English -> Chinese is the primary mature workflow; YouTube/Bilibili links and local videos all let you choose among `English -> Chinese`, `Japanese -> Chinese`, and `Chinese -> English` through the same task pipeline.
- **Selectable output**: Before a task starts, choose subtitles only, translated voice only, or both.
