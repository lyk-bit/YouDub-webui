...
| `SPEAKER_DIARIZATION` | Run speaker diarization after ASR, default `true`. Detected speakers are written to each sentence's `speaker` field; translation and VoxCPM synthesis then keep a distinct voice per speaker. When disabled, every sentence is attributed to speaker `1`. |
