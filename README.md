...
YouDub WebUI 可以把单个 YouTube、Bilibili 或本地视频转换成目标语言版本：下载或导入视频、识别并翻译内容，再按任务选择添加字幕、生成目标语言配音，或者同时完成两者，最后输出可在线播放和下载的新视频。

核心成熟场景是 **YouTube 英文 -> 中文配音**；YouTube、Bilibili 链接和本地视频均可在“英文 -> 中文”“日文 -> 中文”“中文 -> 英文”三种翻译方向中按任务选择（默认值：YouTube 为英文 -> 中文，Bilibili 为中文 -> 英文）。
...
| `LOCAL_UPLOAD_MAX_BYTES` | 本地视频上传大小上限，默认 4 GiB。上传经由 Next 的 `/api` 代理转发，若要调到 8 GiB 以上，需同步调大 [next.config.ts](apps/web/next.config.ts) 的 `experimental.proxyClientMaxBodySize` 并重新构建前端，否则请求体会被静默截断。 |
...
8. 回到首页，提交 YouTube URL、Bilibili URL，或上传本地视频。
   - 在“输出内容”中选择“仅添加字幕”“仅翻译声音”或“字幕和翻译声音”。仅字幕模式保留原声并跳过配音阶段；仅声音模式不会压制字幕。
   - 本地视频可额外上传一份已翻译好的 `.srt` 字幕；上传后会跳过 Whisper 识别和 OpenAI 翻译，再按所选输出内容使用这份字幕。
   - 三种来源都可在“翻译方向”中选择“英文 -> 中文”“日文 -> 中文”或“中文 -> 英文”；未手动选择时按来源使用默认方向（YouTube 英文 -> 中文，Bilibili 中文 -> 英文）。翻译方向也决定本地视频可选字幕的目标语言，例如选择“日文 -> 中文”时，上传的 SRT 会被视为中文字幕。
...
YouTube / Bilibili URL
  -> yt-dlp 下载单个视频
  -> Demucs 分离人声与背景音
  -> Whisper 识别语音并输出词级时间戳（日译中任务默认使用 Kotoba-Whisper）
  -> 句子与时间范围整理
  -> OpenAI 兼容 API 预处理全文并逐句并发翻译
  -> 按输出内容分支：
     - 仅字幕：保留原声并压制翻译字幕
     - 仅声音：VoxCPM2 配音并与背景音混音，不压制字幕
     - 两者：生成配音、混音并压制翻译字幕
  -> FFmpeg 输出最终 mp4
```

配音模式下，语音段落之间的非语音人声（笑声、叹息、呻吟等不会被 Whisper 转写的声音）会从分离出的人声轨回填到配音里，因此这些片段不会变成无人声的空白；低于能量门限的分离残响仍保持静音。

本地视频上传使用同一条后半段流水线，并支持英文、日文识别后翻译为中文，或中文识别后翻译为英文。若同时上传已翻译 `.srt` 字幕，系统会从 SRT 生成内部字幕时间轴，跳过 Whisper 与 OpenAI 翻译阶段，再按所选输出内容继续处理。v1 仅支持本地视频搭配 `.srt`，不支持 URL 任务附加字幕。

## 功能亮点

- **真实可用的端到端流程**：从 URL 到最终视频，不需要手动拆分音频、整理字幕或压制视频。
- **多方向入口**：YouTube 英文 -> 中文是核心成熟场景；YouTube / Bilibili 链接与本地视频都支持在“英文 -> 中文”“日文 -> 中文”“中文 -> 英文”中选择翻译方向，并接入同一条任务流水线。
- **输出自由选择**：任务开始前可选择仅添加字幕、仅生成目标语言配音，或同时完成两者。
